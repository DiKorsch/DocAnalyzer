...
class DocReader(CfbReader):
    ...
    def word_document(self):
        ...
        if not self._word_document:
            ...
            if w_ident != 0xa5ec: #pass
                raise Exception('wIdent is an unsigned integer that specifies '
                    + 'that this is a Word Binary File. This value MUST be 0xA5EC, but was %s.' %(w_ident))
    ...
    def read(self, size=None):
        ...
        for i in range(len(self.cp) - 1):
            ...
            if not fc_f_compressed:
                ...
                buffer += part.encode('utf-16')
            else:
                part = part.decode('latin-1')
                buffer += part.encode('latin-1')
            self._position += len(part)

            if len(buffer) >= size:
                break

        RESULT = buffer[:size].replace(chr(160), chr(32)).replace(chr(13), chr(10))
        # try:
        #     RESULT = RESULT.decode('latin-1').encode('utf-8')
        # except Exception, e:
        #     for c in RESULT:
        #         print c, "===>", ord(c)
        #     raise e
